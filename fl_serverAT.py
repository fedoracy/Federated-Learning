...
from Paras import GPU_MEMORY_FRACTION_P, MIN_NUM_WORKERS_P, MAX_NUM_ROUNDS_P, NUM_CLIENTS_CONTACTED_PER_ROUND_P, ROUNDS_BETWEEN_FULL_COMMUNICATION_P, POWER_A_P
from Paras import DATA_SPLIT_P, EPOCH_PER_ROUND_P, BATCH_SIZE_P, POINT_TRANSFER_P
...
class GlobalModel(object):
    ...
    def __init__(self):
        ...
        self.current_weights4_test_and_eval = self.current_weights.copy()
    ...
    def update_weights_transfer(self, client_weights, client_sizes):
        new_weights = [np.zeros(w.shape) for w in self.current_weights]
        total_size = np.sum(client_sizes)
        for c in range(len(client_weights)):
            for i in range(len(new_weights)):
                new_weights[i] += client_weights[c][i] * client_sizes[c] / total_size
        self.current_weights[:self.point_transfer] = new_weights[:self.point_transfer]
...
class FLServer(object):
    ...
    def __init__(self, global_model, host, port):
        ...
        self.weight_states_dic = {}
        self.weight_states_transfer_dic = {}
        self.current_round = -1
        self.client_updates = []
        self.count_every_commu_round = 0
        self.count_every_full_commu_round = 0
    ...
    def register_handles(self):
        ...
        def handle_client_update(data):
            print("received client update of bytes: {} \nhandle client_update: {}".format(sys.getsizeof(data), request.sid))
            if data['round_number'] == self.current_round:
                data.update({'weights': pickle_string_to_obj(data['weights'])})
                self.weight_states_dic.update({request.sid: {'round_stamp': data['round_number'], 'data': data}})
                self.count_every_commu_round += 1
                if data['full_communication']:
                    self.weight_states_transfer_dic.update(
                        {request.sid: {'round_stamp': data['round_number'], 'data': data}})
                    self.count_every_full_commu_round += 1
                if self.count_every_commu_round >= FLServer.NUM_CLIENTS_CONTACTED_PER_ROUND:
                    self.obtain_client_updates_a()
                    self.count_every_commu_round = 0
                    self.global_model.update_weights_transfer(
                        [x['weights'] for x in self.client_updates],
                        [x['train_size_update_weights'] for x in self.client_updates],
                    )
                    aggr_train_loss, aggr_train_accuracy = self.global_model.aggregate_train_loss_accuracy(
                        [x['train_loss'] for x in self.client_updates],
                        [x['train_accuracy'] for x in self.client_updates],
                        [x['train_size'] for x in self.client_updates],
                        self.current_round
                    )
                    ...
                    if self.count_every_full_commu_round >= FLServer.NUM_CLIENTS_CONTACTED_PER_ROUND:
                        self.obtain_client_updates_transfer_a()
                        self.count_every_full_commu_round = 0
                        self.global_model.update_weights(
                            [x['weights'] for x in self.client_updates_transfer],
                            [x['train_size_update_weights'] for x in self.client_updates_transfer],
                        )
                    if len(self.current_round_client_updates) >= FLServer.NUM_CLIENTS_CONTACTED_PER_ROUND:
                        ...
                        self.current_round_client_updates = []
                        print("aggr_valid_loss: {} \naggr_valid_accuracy: {}".format(aggr_valid_loss, aggr_valid_accuracy))
                        self.test_and_eval()
        ...
        def handle_client_eval(data):
            ...
            if len(self.eval_client_updates) == self.MIN_NUM_WORKERS:
    ...
    def obtain_client_updates_a(self):
        ...
        for i in self.weight_states_dic.values():
            ...
            self.client_updates += [i['data']]
            if dis_round == 0 and 'valid_loss' in i['data']:
                self.current_round_client_updates += [i['data']]
    ...
    def train_next_round(self):
        ...
        for rid in client_sids_selected:
            emit('request_update', {
                    ...
                    'full_communication_validation': self.current_round % FLServer.ROUNDS_BETWEEN_FULL_COMMUNICATION[0]
                                                        in FLServer.ROUNDS_BETWEEN_FULL_COMMUNICATION[1],
            }, room=rid)
    ...
    def test_and_eval(self):
        ...
        for rid in self.ready_client_sids:
            emit('test_and_eval', {
                'model_id': self.model_id,
                'current_weights': obj_to_pickle_string(self.global_model.current_weights4_test_and_eval),
                'weights_format': 'pickle'
                }, room=rid)
