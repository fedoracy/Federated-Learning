import pickle, keras, uuid, random, codecs, json, time
from keras.models import Sequential
from keras.layers import Dense, Flatten, Conv2D, MaxPooling2D
import numpy as np
from flask import *
from flask_socketio import *
from Paras import MIN_NUM_WORKERS_P, NUM_CLIENTS_CONTACTED_PER_ROUND_P
from Paras import GPU_MEMORY_FRACTION_P, MAX_NUM_ROUNDS_P, ROUNDS_BETWEEN_VALIDATIONS_P
...
class FLServer(object):
    ...
    def register_handles(self):
        ...
        def handle_client_update(data):
            ...
            if data['round_number'] == self.current_round:
                ...
                if self.num_client_updates >= FLServer.NUM_CLIENTS_CONTACTED_PER_ROUND:
                    self.obtain_current_round_and_client_updates()
                    self.num_client_updates = 0
                    ...
                    print('^^^^^******^^^^^   len: {}'.format(len(self.client_updates)))
                    for x in self.client_updates:
                        print('^^^^^******^^^^^: {}'.format(x['round_number']))
    ...
    def obtain_current_round_and_client_updates(self):
        self.client_updates = []
        for i in self.weight_states_dic.values():
            self.client_updates += [i['data']]
            if 'valid_loss' in i['data'] and self.current_round == i['round_stamp']:
